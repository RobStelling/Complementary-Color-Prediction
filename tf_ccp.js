...
function updateUI() {
  ...
  // Adds one color to a colorVector
  ...
  for (let i = 0; i < sampleList.length; i++) {
    addColor(d3.select("#pr"+sampleList[i]).style("fill"), sampledColors[i]);
    d3.select("#grad"+i).selectAll("stop").remove();
    var stops = d3.select("#grad"+i).selectAll("stop").data(sampled2stops(sampledColors[i]));
    stops.exit().remove();
...
function initializeUi() {
  ...
  for (let i = 0; i < sampleList.length; i++) {
    ...
    // Target color
    svg.append("rect")
      .style("fill", function(){return d3.select("#co"+sampleList[i]).style("fill");})
    ...
    gradStops.push(sampled2stops([d3.select("#pr"+sampleList[i]).style("fill")]));
    var gradient = d3.select("#Gdefs").append("linearGradient")
                      .attr("id", "grad"+i)
                      .attr("x1", "0%")
                      .attr("x2", "100%")
                      .attr("y1", "0%")
...
function resetEnvironment() {
  ...
  for (let i = 0; i < sampleList.length; i++) {
    d3.select("#grad"+i).selectAll("stop").remove();
    var gradStops = sampled2stops([d3.select("#pr"+sampleList[i]).style("fill")]);
    var gradient = d3.select("#grad"+i);
...
function setInterfaceHooks() {
  ...
  function mouseoverSVG(d) {
    if (d3.select(this).classed("predicted"))
      ...
      tooltipSVG.html("Reference color");
